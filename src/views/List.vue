    ...
    <div class="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
      <div  class="overflow-y-auto h-screen  rounded-lg md:w-2/3">
        <div v-for="item in carrinho" :key="item.id" class="justify-between mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start">
          <img :src="'https://raw.githubusercontent.com/ThiagoRazor/tela-produtos/main/src/assets/img/'+ item.id + '.bmp'" alt="product-image" class="w-full rounded-lg sm:w-40" />
          <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between">
            <div class="mt-5 sm:mt-0">
              <h2 class="text-lg font-bold text-gray-900">{{ item.nomeProduto }}</h2>
            </div>
        ...
        <div class="container px-6 py-8 mx-auto">
            <div class="lg:flex lg:-mx-2">
                <div  class=" mt-24 space-y-3 lg:w-1/5 lg:px-2 lg:space-y-4" :class="{'hidden': !menuOpen}">
                    ...
                    <a @click="carregarProdutos('todos')" class="cursor-pointer block font-medium text-blue-500.active dark:text-gray-300 hover:underline">Todos</a>
                    <a @click="trocarTela('controles')" class="cursor-pointer block font-bold text-yellow-500 uppercase hover:underline ">Controles</a>
                    <a @click="carregarProdutos('carregador')" class="cursor-pointer block font-medium text-gray-500 dark:text-gray-300 hover:underline">Carregadores</a>
                    <a @click="carregarProdutos('cabos')" class="cursor-pointer block font-medium text-gray-500 dark:text-gray-300 hover:underline">Cabos</a>
                    <a @click="carregarProdutos('fone')" class="cursor-pointer block font-medium text-blue-600 dark:text-blue-500 hover:underline">Fones de Ouvido</a>
                    <a @click="carregarProdutos('midias')" class="cursor-pointer block font-medium text-gray-500 dark:text-gray-300 hover:underline">Midias</a>
                    <a @click="carregarProdutos('gamer')" class="cursor-pointer block font-medium text-gray-500 dark:text-gray-300 hover:underline">Gamer</a>
                    <a @click="carregarProdutos('informatica')" class="cursor-pointer block font-medium text-gray-500 dark:text-gray-300 hover:underline">Informatica</a>
                    <a @click="carregarProdutos('adaptadores')" class="cursor-pointer block font-medium text-gray-500 dark:text-gray-300 hover:underline">Adaptadores e Conversores</a>
                    <a @click="carregarProdutos('antenas')" class="cursor-pointer block font-medium text-gray-500 dark:text-gray-300 hover:underline">Antenas</a>
                    <a @click="carregarProdutos('fontes')" class="cursor-pointer block font-medium text-gray-500 dark:text-gray-300 hover:underline">Fontes</a>
                    <a @click="carregarProdutos('eletrica')" class="cursor-pointer block font-medium text-gray-500 dark:text-gray-300 hover:underline">Elétrica</a>
                    <a @click="carregarProdutos('iluminacao' )" class="cursor-pointer block font-medium text-gray-500 dark:text-gray-300 hover:underline">Iluminação</a>
                    <a @click="carregarProdutos('baterias')" class="cursor-pointer block font-medium text-gray-500 dark:text-gray-300 hover:underline">Baterias</a>
                    <a @click="carregarProdutos('ferramentas')" class="cursor-pointer block font-medium text-gray-500 dark:text-gray-300 hover:underline">Ferramentas</a>
                    <a @click="carregarProdutos('suportes')" class="cursor-pointer block font-medium text-gray-500 dark:text-gray-300 hover:underline">Suportes</a>
                    <a @click="carregarProdutos('variedades')" class="cursor-pointer block font-medium text-gray-500 dark:text-gray-300 hover:underline">Variedades</a>
                    <a @click="carregarProdutos('veicular' )" class="cursor-pointer block font-medium text-gray-500 dark:text-gray-300 hover:underline">Veicular</a>
                    <a @click="carregarProdutos('radios')" class="cursor-pointer block font-medium text-gray-500 dark:text-gray-300 hover:underline">Rádios</a>
                    <a @click="carregarProdutos('caixas')" class="cursor-pointer block font-medium text-gray-500 dark:text-gray-300 hover:underline">Caixas de som</a>
                    <a @click="carregarProdutos('eletronicos')" class="cursor-pointer block font-medium text-gray-500 dark:text-gray-300 hover:underline">Eletrônicos</a>
                ...
                <div class="mt-6 lg:mt-0 lg:px-2 lg:w-4/5 ">
                    ...
                    <div class="z-0 grid grid-cols-1 gap-8 mt-8 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        ...
                        class="flex flex-col items-start justify-center w-full max-h-3/6 max-w-lg mx-auto">
                            <img class="bg-white object-contain w-full rounded-md h-48 xl:h-72" :src="'https://raw.githubusercontent.com/ThiagoRazor/tela-produtos/main/src/assets/img/'+ produto.id + '.bmp'" alt="T-Shirt">
                            <h2 class="font-bold w-14 text-center mt-4 rounded text-slate-600 bg-orange-300">Nº: {{ produto.id }}</h2>
                            <h4 class="mt-2 h-16 text-md font-medium text-gray-700 dark:text-gray-200">{{ produto.nomeProduto }}</h4>
...
export default {
  ...
  methods: {
    ...
    fetch("https://raw.githubusercontent.com/ThiagoRazor/tela-produtos/main/src/data/produtos.json?token=GHSAT0AAAAAAB6U2YDPR3QRZL3YEL5OYE2UZBT53RA")
    ...
    async finalizarCompra() {
          ...
          if (item && item.nomeProduto) { // verificar se item não é nulo ou indefinido
            pdf.setFontSize(10);
            pdf.text(`${item.nomeProduto} Qtd (${item.quantidade}x) - R$ ${item.total.toFixed(2)}`, 10, y);
            y += 10;
          }
          });
